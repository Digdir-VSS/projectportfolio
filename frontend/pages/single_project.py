...
from models.validators import to_json, to_list, to_date_str, convert_to_int, convert_to_float, add_thousand_split, convert_to_int_from_thousand_sign, sort_selected_values, to_datetime
...
def show_projects(projects, email: str, access_allowance: bool):
...
def project_detail(prosjekt_id: str, email: str, project: ProjectData, brukere_list, access_allowance: bool = True):
    ...
    with ui.grid(columns=5).classes("w-full gap-5 bg-[#f9f9f9] p-4 rounded-lg"):
        ...
        with ui.element("div").classes('col-span-2 row-span-1 col-start-2 row-start-3'):
            ...
            lock(ui.input(value=project.portfolioproject.navn)
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.portfolioproject, "navn"))
        with ui.element("div").classes('col-span-3 row-span-1 col-start-1 row-start-4'):
            ...
            lock(ui.select(list(brukere_list.keys()), with_input=True, multiple=False,
                    validation=lambda value: "Du må velge en tiltakseier" if value is None else None)
                .props("outlined dense clearable options-dense color=primary")
                .classes("w-full bg-white rounded-lg")
                .props('use-chips')
                .bind_value(project.portfolioproject, "tiltakseier"))
        with ui.element("div").classes('col-span-1 row-span-1 col-start-4 row-start-4'):
            ...
            lock(ui.select(FASE, value=None)
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.fremskritt, "fase"))
        with ui.element("div").classes('col-span-3 row-span-1 col-start-1 row-start-5'):
            ...
            lock(ui.select(list(brukere_list.keys()), with_input=True, multiple=True, on_change=sort_selected_values)
                .props("clearable options-dense color=primary")
                .classes("w-full bg-white rounded-lg")
                .props('use-chips')
                .bind_value(project.portfolioproject, "kontaktpersoner", forward=to_json, backward=to_list))
        with ui.element("div").classes('col-span-1 row-span-1 col-start-4 row-start-5'):
            ui.label('Start').classes('text-lg font-bold')
            lock(ui.input()
                .bind_value(project.portfolioproject, "oppstart", backward=to_date_str, forward=to_datetime)
        ...
        with ui.element("div").classes('col-span-1 row-span-1 col-start-5 row-start-5'):
            ...
            lock(ui.input()
                .bind_value(project.fremskritt, "planlagt_ferdig", backward=to_date_str, forward=to_datetime)
                .props("outlined dense type=date clearable color=primary")
                .classes("w-full"))
        with ui.element("div").classes('col-span-3 row-span-1 col-start-1 row-start-6'):
            ...
            lock(ui.radio(avdelinger)
                .props("inline")
                .bind_value(project.portfolioproject, "avdeling"))
        with ui.element("div").classes('col-span-2 row-span-1 col-start-1 row-start-7'):
            ...
            lock(ui.select(avdelinger, multiple=True, on_change=sort_selected_values)
                .props("use-chips")
                .classes("w-full bg-white rounded-lg")
                .bind_value(project.samarabeid, "samarbeid_intern", forward=to_json, backward=to_list))
        with ui.element("div").classes('col-span-1 row-span-1 col-start-3 row-start-7'):
            ...
            lock(ui.input()
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.samarabeid, "samarbeid_eksternt"))
        with ui.element("div").classes('col-span-3 row-span-2 col-start-4 row-start-6'):
            ...
            lock(ui.textarea()
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.samarabeid, "avhengigheter_andre"))
    ...
    with ui.grid(columns=5).classes("w-full gap-5 bg-[#f9f9f9] p-4 rounded-lg"):
        ...
        with ui.element("div").classes('col-span-5 row-span-3 col-start-1 row-start-3'):
            ...
            lock(ui.textarea(value=project.problemstilling.problem)
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.problemstilling, "problem"))
        with ui.element("div").classes('col-span-5 row-span-3 col-start-1 row-start-6'):
            ...
            lock(ui.textarea(value=project.tiltak.tiltak_beskrivelse)
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.tiltak, "tiltak_beskrivelse"))
        with ui.element("div").classes('col-span-5 row-span-2 col-start-1 row-start-9'):
            ...
            lock(ui.textarea(value=project.risikovurdering.vurdering)
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.risikovurdering, "vurdering"))
    ...
    with ui.grid(columns=4).classes("w-full gap-5 bg-[#f9f9f9] p-4 rounded-lg"):
        ...
        lock(ui.textarea()
            .classes('col-span-2 row-span-2 col-start-1 row-start-5 bg-white rounded-lg')
            .bind_value(project.malbilde, "malbilde_1_beskrivelse"))
        ...
        lock(ui.textarea()
            .classes('col-span-2 row-span-2 col-start-3 row-start-5 bg-white rounded-lg')
            .bind_value(project.malbilde, "malbilde_2_beskrivelse"))
        ...
        lock(ui.textarea()
            .classes('col-span-2 row-span-2 col-start-1 row-start-8 bg-white rounded-lg')
            .bind_value(project.malbilde, "malbilde_3_beskrivelse"))
        ...
        lock(ui.textarea()
            .classes('col-span-2 row-span-2 col-start-3 row-start-8 bg-white rounded-lg')
            .bind_value(project.malbilde, "malbilde_4_beskrivelse"))
        with ui.element("div").classes('col-span-4 row-span-5 col-start-1 row-start-10'):
            ...
            lock(ui.select(DIGITALISERINGS_STRATEGI, multiple=True, on_change=sort_selected_values)
                .classes('w-full bg-white rounded-lg')
                .props('use-chips')
                .bind_value(project.digitaliseringstrategi, "sammenheng_digital_strategi", forward=to_json, backward=to_list))
        with ui.element("div").classes('col-span-4 row-span-2 col-start-1 row-start-15'):
            ...
            lock(ui.textarea()
                .bind_value(project.digitaliseringstrategi, "digital_strategi_kommentar")
                .classes('w-full bg-white rounded-lg'))
    ...
    with ui.grid(columns=5).classes("w-full gap-5 bg-[#f9f9f9] p-4 rounded-lg"):
        ...
        with ui.element("div").classes('col-span-2 row-span-2 col-start-1 row-start-3'):
            ...
            lock(ui.textarea()
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.resursbehov, "kompetanse_som_trengs"))
        with ui.element("div").classes('col-span-1 row-span-1 col-start-3 row-start-3'):
            ...
            lock(ui.select(["Ja", "Ja, men det er ikke tilstrekkelig kapasitet", "Delvis", "Nei"], value=None)
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.resursbehov, "kompetanse_tilgjengelig"))
        ...
        with ui.element("div").classes('col-span-1 row-span-1 col-start-4 row-start-3'):
            ...
            lock(ui.input()
                ...
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.resursbehov, "antall_mandsverk_intern", forward=convert_to_float))
        with ui.element("div").classes('col-span-1 row-span-1 col-start-5 row-start-3'):
            ui.label("Eksterne").classes('text-lg font-bold')
            lock(ui.input()
                .props('type=number min=0 step=0.1')
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.resursbehov, "antall_mandsverk_ekstern", forward=convert_to_float))
    ...
    with ui.grid(columns=5).classes("w-full gap-5 bg-[#f9f9f9] p-4 rounded-lg"):
        ...
        with ui.element("div").classes('col-span-2 row-span-1 col-start-1 row-start-3'):
            ...
            lock(ui.input()
                ...
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.resursbehov, "estimert_budsjet_behov",
                            backward=add_thousand_split, forward=convert_to_int_from_thousand_sign))
            ...
            with ui.element("div").classes("flex flex-wrap space-x-8 mt-2"):
                for year in [2026, 2027, 2028]:
                    ...
                    with ui.element("div").classes("flex flex-row items-center"):
                        ...
                        lock(ui.input()
                            .props('inputmode=numeric min=0 step=1 input-style="text-align: right;"')
                            .classes('w-24 bg-white rounded-lg')
                            .bind_value(project.ressursbruk[year], 'predicted_resources',
                                        backward=add_thousand_split,
                                        forward=convert_to_int_from_thousand_sign))
        with ui.element("div").classes('col-span-1 row-span-1 col-start-1 row-start-4'):
            ...
            lock(ui.select(ESTIMAT_LISTE, value=None)
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.resursbehov, "risiko_av_estimat"))
        with ui.element("div").classes('col-span-4 row-span-2 col-start-2 row-start-4'):
            ...
            lock(ui.textarea(value=project.resursbehov.estimert_budsjet_forklaring)
                .classes('w-full bg-white rounded-lg')
                .bind_value(project.resursbehov, "estimert_budsjet_forklaring"))
