...
from models.ui_models import OpenOverviewUI
...
def open_overview_page(overview: List[OpenOverviewUI]):

    with ui.column().classes("w-full gap-2"):

        columns = create_columns(OpenOverviewUI.model_fields)
    ...
    table.add_slot(
        "header",
        r"""
        <q-tr :props="props" class="bg-gray-200">
            <q-th v-for="col in props.cols" :key="col.name" :props="props"
                class="bg-gray-200 text-lg font-semibold">
                {{ col.label }}
            </q-th>
        </q-tr>
        """,
    ...
    table.add_slot(
             "body-cell-kontaktpersoner",
        r"""
        <q-td :props="props">
            {{
                props.value
                    ? JSON.parse(props.value).join('; ')
                    : ''
            }}
        </q-td>
        """
